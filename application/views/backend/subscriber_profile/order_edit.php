<div id="main-content">
  ...
  <div class="container-fluid">
    ...
    width: 298px;
                  ...
                  <input type="hidden" class="oid" name="oid" value="<?php echo $this->uri->segment(3);?>">

               </div>

              <div class="form-group">

                <label class="control-label" >Copies Requested (Please enter number of copies you needed)</label>

                  <div class="col-lg-10 col-md-10 col-lg-offset-1">

                    <input type="text" class="form-control" id="Copies_requested" maxlength="3" name='Copies_requested' value='<?php echo $orders['No_of_copies'];?>' onkeyup="getDiscount($(this).val(),'')"  placeholder="Please enter number copies" autocomplete="off">
              ...
              <div class="form-group">
                ...
                <div class="col-md-10 col-lg-offset-1">
                  ...
                  <select id="request_month" name='Request_month' class="form-control" onchange="getDiscount($('#Copies_requested').val(),$(this).val())">
                    ...
                    <option value="1" <?php if($orders['subscription_length'] == 1) echo 'selected';?>>1</option>

                    <option value="3" <?php if($orders['subscription_length'] == 3) echo 'selected';?>>3</option>

                    <option value="6" <?php if($orders['subscription_length'] == 6) echo 'selected';?>>6</option>

                    <option value="9" <?php if($orders['subscription_length'] == 9) echo 'selected';?>>9</option>

                    <option value="12" <?php if($orders['subscription_length'] == 12) echo 'selected';?>>12</option>
...

         {
...

          {
...

            {
              ...
              $('#message_discount').html("<div class='alert alert-dismissible alert-success'>Your total amount is $"+(amount)+"</div>");
          ...
          $.ajax({
              ...
              data:"copy="+copy+"&amount="+amount+"&oid="+oid,

              url:"subscriber_profile/get_refund",

              success:function getstate($msg)

              {
//console.log($msg.split('#'));
                if($msg.split('#')[0] == 'extra')
                {
                  $('#message_discount').append("<div class='alert alert-dismissible alert-success'>You need to pay only $"+($msg.split('#')[1])+". Your previous order amount $"+$msg.split('#')[2]+"</div>");
                  $('.payment').show();
                }
                if($msg.split('#')[0] == 'refund')
                {
                  $('#message_discount').append("<div class='alert alert-dismissible alert-success'>You will get refund of $"+($msg.split('#')[1])+". Your previous order amount $"+$msg.split('#')[2]+"</div>");

                  $("#optionspayment2").prop("checked", true);
                  $('.payment').hide();

                }
                
                if($msg.split('#')[0] == 'normal')
                {
                  $('#message_discount').append("<div class='alert alert-dismissible alert-success'>You need to pay $"+($msg.split('#')[1])+"</div>");
                  $('.payment').show();
                }
