...
if($this->Subscriber_model->order_status($row['id']) == 1) {
        echo '<td>'.$this->Subscriber_model->subscription_status($row['id']).' / '.ucfirst($this->Subscriber_model->validate_subscription($row['id'])).'</td>';
      }
...
if($this->Subscriber_model->expire_status($row['id'])=='1') { 
         echo ' <a href="'.base_url().'subscriber_profile/order_edit/'.sha1($row['id']).'" class="btn btn-success">MODIFY</a>';
}
          echo '&nbsp;<a href="'.base_url().'subscriber_profile/order_details/'.sha1($row['id']).'" class="btn btn-danger">DETAILS</a>';
          echo '&nbsp;<a href="'.base_url().'subscriber_profile/order_history/'.sha1($row['id']).'" class="btn btn-danger">HISTORY</a>';
if($this->Subscriber_model->expire_status($row['id'])=='1') { 
          echo '&nbsp;<a href="'.base_url().'subscriber_profile/order_cancel/'.sha1($row['id']).'" class="btn btn-success">CANCEL</a>';
}
          echo '</td>';
        }
          else
