...
class State_zone extends CI_Controller {
	...
	public function index()
	{
		 ...
		 $data['title'] = 'State Zone';
         $data['menu']  = 'state_zone';
         $data['content'] = 'backend/state_zone/home';
	...
	public function county_list()
	{
		if($this->uri->segment(3)!='')
		{
			...
			$data['title'] 		= 'State Zone';
            $data['menu']		= 'state_zone';
            $data['content'] 	= 'backend/state_zone/county_list';
            $this->load->view( 'backend/template', $data );
		}
		else
		{
	...
	public function state_zone_list()
	{
		...
		{
			$id   = $this->uri->segment(3);
			$data['county'] = $this->common->state_zone_list($id);
			$data['title'] = 'State';
            $data['menu'] = 'state';
            $data['content'] = 'backend/state/state_zone_list';
            $this->load->view( 'backend/template', $data );
		}
		else
		{
	...
	public function distributers_list()
	{
		if($this->uri->segment(3)!='')
		{
			...
			$data['distributer'] = $this->common->distributer_list_all($id); 
		 	$data['title'] = 'State Zone';
         	$data['menu'] = 'state_zone';
         	$data['content'] = 'backend/state_zone/distributer_list';
         	$this->load->view( 'backend/template', $data );
		}
		else
		{
	...
	public function subscribers_list()
	{
		if($this->uri->segment(3)!='')
		{
			$id = $this->uri->segment(3);
		 	...
		 	$data['title'] = 'State Zone';
        	$data['menu'] = 'state_zone';
         	$data['content'] = 'backend/state_zone/subscribers_list';
        	$this->load->view( 'backend/template', $data );
		}
	 	else
		{
