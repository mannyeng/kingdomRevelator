...
class Distributer_model extends CI_model {
    ...
    public function order_add($data = "")
    {
             ...
             $this->db->trans_start();
             $this->db->insert("kr_dis_orders",$data);
    ...
    {
       $data['Subscriber_id'] = $data['Distributer_id'];
      unset($data['Distributer_id']);
           $this->db->trans_start();
             $this->db->insert("kr_dis_payment",$data);
             $id = $this->db->insert_id();
             $this->db->trans_complete();
  ...
  public function order_update($data = "")
    {
      ...
      $this->db->where('Distributer_id', $data['Distributer_id']);
      $this->db->update('kr_dis_orders', $param);
      $this->db->trans_complete();
             if ($this->db->trans_status() === FALSE)
        {
          return false;
...
//pr($param);
      ...
      $this->db->update('kr_dis_payment', $param);
      $this->db->trans_complete();
             if ($this->db->trans_status() === FALSE)
        {
          return false;
  ...
  // to get all orders
    ...
    {
        //echo $id;
       $resp =  $this->db->query("SELECT o.id,o.Distributer_id,o.subscription_length,o.No_of_copies,o.Cash_Check_by,o.comments,o.subscription_date,o.expiry_date FROM `kr_dis_orders` o where o.Distributer_id='$id' order by o.id DESC")->result_array();
...
// to get detail order
    ...
    {
       //echo $orderid;
       $resp =  $this->db->query("SELECT o.id,o.subscription_length,o.No_of_copies,o.Cash_Check_by,p.paypal_status,o.comments,o.subscription_date,o.expiry_date,o.status,SUM(CASE WHEN p.paypal_status='completed' THEN p.paid_amnt ELSE 0 END) totamnt,SUM(CASE WHEN p.refund_status = '1' THEN p.refund_amnt ELSE 0 END) refundamnt FROM `kr_dis_orders` o left join kr_dis_payment p on(o.id=p.order_id) where o.Distributer_id='$id' and p.Subscriber_id='$id' and sha1(o.id) ='".$orderid."'  group by p.order_id")->row_array();
       //echo $this->db->last_query();
     return $resp;
    ...
    public function payment_details($paymentid)
    {
      ...
      $resp =  $this->db->query("SELECT * FROM `kr_dis_payment`  where id=$paymentid")->row_array();
      // echo $this->db->last_query();
     return $resp;
    }
...
// update payment details from admin for cash/cheque
  public function refund_update_process($data = "")
    ...
    public function order_status($orderid)
    {
       ...
       $res = $this->db->select('status')->get_where("kr_dis_orders",array('id'=>$orderid))->row()->status;
     return $res;
    }
    ...
    public function get_refund($copy,$amount,$orderid)
    {
        ...
        $resp = $this->db->query("SELECT o.*, p.paid_amnt FROM `kr_dis_payment` p left join kr_dis_orders o on (o.id=p.order_id) where p.id=(select max(id) from kr_dis_payment where paypal_status='completed' and sha1(order_id)='".$orderid."') and o.status=1 ")->row_array();
        //echo $this->db->last_query();
//pr($resp);
        ...
        if($substart->format('%R%a') >=30)
        {
            ...
            if($substartstaus->format('%R%a') < 0)
            {
              ...
              $datetime4 = new DateTime($nxtmnth);
                ...
                $price_for_used  = ($newdifference*$price);
    ...
    public function subscription_status($orderid)
    {
       ...
       $dte = $this->db->select('expiry_date,subscription_date')->get_where("kr_dis_orders",array('id'=>$orderid))->row_array();
       $cur = date('Y-m-d');
...
return $x;
 ...
 // function to save order history
    ...
    public function save_history($orderid)
    {
       ...
       $data =  $this->db->query("SELECT o.id order_id,o.No_of_copies,o.subscription_length,o.subscription_date,o.expiry_date,o.created_date order_date,p.Mode_of_pay FROM `kr_dis_orders` o left join kr_dis_payment p on (p.order_id = o.id) where p.id = (select max(id) from kr_dis_payment where order_id=$orderid) and o.id=$orderid")->row_array(); 
 
       $this->db->trans_start();
             $this->db->insert("kr_dis_order_history",$data);
              $this->db->trans_complete();
...
// order history list
    public function order_history($orderid)
    {
       ...
       $data =  $this->db->query("SELECT * FROM kr_dis_order_history where sha1(order_id)='".$orderid."' order by id DESC")->result_array(); 
...
/// to check he has any valid subscription with orderid
    ...
    public function validate_subscription($orderid)
    {
      ...
      $dte = $this->db->select('expiry_date,subscription_date')->get_where("kr_dis_orders",array('id'=>$orderid,'status'=>'1'))->row_array();
      ...
      if(count($dte)>0)
      {
       ...
       {
           //active
          $resp =  $this->db->query("SELECT paypal_status from kr_dis_payment where id = (select max(id) from kr_dis_payment where order_id=$orderid) and order_id=".$orderid)->row()->paypal_status;
        //echo $this->db->last_query();
